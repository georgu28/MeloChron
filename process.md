...
A running record of the MeloChron **adoption** effort: the plan, every decision
and its reasoning, the results, and what those results *mean*. Written for
review. The spine is the phases in build order (0 → 5); Phases 0–3 are done,
4–5 are planned. Cross-cutting engineering (hardware, `torch.compile`, the swap
fix, the scorer) is captured inside Phase 3, where it happened.
...
**Two deliverables (the brief's).** (1) aggregate PR-AUC per slice vs base rate;
(2) a prediction demo — real users, their first-encountered tracks, predicted
adoption probability, and what actually happened.
...
| **5** | The demo (real users, their first encounters, predicted prob, what happened) + fold results into `README.md` | demo tables, README | ▶ next |
...
1. **pure vs priors:** split pass — pure wins cold_user (sig), priors wins
   overall + unfamiliar (sig); no single head wins both required slices.
2. **max_len:** no conclusion changes; history helps `pure`, not `priors`.
3. **time on/off:** time **significantly helps** (5/6) — unlike the prior project.
4. **ID vs hybrid:** ID beats *hybrid* everywhere (the residual overfits); the
   ranking cold-item catastrophe **does not transfer** to binary adoption. ⚑ *But
   "content is weak" is later retracted* — see Audio features: residual-free
   content (`text_frozen`) beats ID, and learned audio beats genre.
5. **event-N:** ranking stable across horizon.
6. **Audio features:** content is *not* weak — `text_frozen` content beats ID
   (+arch, +0.03\*) and audio beats genre (+0.03\*); the best content model ties
   the in-context rate and covers cold-start.

**The through-line (revised twice — after the cold-start adjudication, then the
audio-features probe):**
- A training-free **in-context running rate** is the single strongest signal — it
  beat every ID model, overall and on cold_user (Cold-start adjudication).
- **Content is not weak** (the initial finding is retracted): a residual-free
  `text_frozen` projection of content, run through the encoder, beats every ID
  variant and — with learned audio — **ties the in-context rate** while needing no
  ID table (Audio features). The item representation *does* matter after all; the
  earlier "content weak / ID wins" came from raw cosine and the residual hybrid.
- Time-delta genuinely helps (a real divergence from the prior project).
- **Honest bottom line:** the best model is a content-based sequence model that
  *matches* but does not *beat* the two-line in-context rate. The strongest,
  most-defensible results are the rigorous *analysis* (in-context rate is the bar;
  content weakness was a method artifact; audio > genre; time matters), not a model
  that decisively beats the simple baseline.
...
## Phase 5 — the demo & write-up  ▶ in progress

- **Demo — ✅ done.** `scripts/demo_adoption.py` → `artifacts/adoption/phase5-demo.md`.
  Reads the dumped id-priors probabilities (real checkpoint outputs) joined to the
  label table, and shows three real users spanning the slices:
  - **heavy user** (#42346, 5,616 first-encounters, only 5% adopted — a music
    explorer): model correctly predicts uniformly low, but separates
    adopted-vs-not by only 0.09 vs 0.08.
  - **cold user** (#57798, no history, 31% adopted): model leans on in-context
    signal, predicts ~0.4 (roughly calibrated), weak separation 0.41 vs 0.40.
  - **new-neighbourhood listener** (#47389, 35% adopted, high-adopter prior 0.49):
    predicts high (0.6–0.75), 4 of top-8 returned, separation 0.60 vs 0.56.

  **Meaning:** at the *individual* level the model separates returns from
  non-returns only weakly (Δ ≈ 0.01–0.04 in predicted prob) — honestly consistent
  with the modest aggregate lift (1.2–1.3×). It is a real signal, not a strong one.
  Tracks show as Onion ids (no titles/artists in the dataset).

- **Write-up — ▶ next.** Fold headline results into `README.md` (results live in
  README; `artifacts/` is gitignored). Every PR-AUC beside its base rate;
