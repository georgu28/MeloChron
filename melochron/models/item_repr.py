...
def build_item_representation(
    ...
    the three rows of the Phase 2 ablation table, so a config file can select a
    row by name and the results table can be regenerated by one command.
    """
    if variant == "id":
